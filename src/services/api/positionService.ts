...
import { ammWagmiConfig, getPublicClient } from "./ammClient";
...
async function getHighestTokenId(pc: any): Promise<bigint> {
  ...
  let low = 1n;
  let high = 1000n; // reasonable initial upper bound
  let highest = 0n;

  // Exponential search for upper bound
  while (true) {
    try {
      await readContract(pc, { address, abi: erc721Abi, functionName: 'ownerOf', args: [high] });
      low = high;
      high *= 2n;
    } catch (e) {
      break;
  ...
  // Binary search for exact highest token ID
  while (low <= high) {
    const mid = (low + high) / 2n;
    try {
      await readContract(pc, { address, abi: erc721Abi, functionName: 'ownerOf', args: [mid] });
      highest = mid;
      low = mid + 1n;
    } catch (e) {
      high = mid - 1n;
...
async function ownedPositionTokenIds(user: Address): Promise<bigint[]> {
  ...
  // Amoy RPC does not support large block ranges for getLogs.
  // Since NonfungiblePositionManager lacks tokenOfOwnerByIndex,
  // we binary search for the highest minted token ID and check ownership.
  ...
  // Batch ownerOf checks (Promise.allSettled is safe for parallel RPC)
