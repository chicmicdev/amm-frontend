...
async function ownedPositionTokenIds(user: Address): Promise<bigint[]> {
  ...
  // let iterations = 0;
  // for (let to = latest; ;) {
  //   if (iterations++ > 4) break; // Limit block traversal to prevent RPC prune errors
  ...
  //   } catch (error: any) {
  //     if (
  //       error?.message?.toLowerCase().includes("pruned") ||
  //       error?.message?.toLowerCase().includes("limit")
  //     ) {
  //       console.warn("RPC history limit reached, stopping log search earlier.");
  //       break;
  //     }
  //     throw error;
  //   }
