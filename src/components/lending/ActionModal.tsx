...
export function ActionModal({ reserve, mode, userAddress, onClose, onSuccess }: ActionModalProps) {
  ...
  const maxWei = useMemo(() => {
    ...
    switch (mode) {
      ...
      case 'withdraw': {
        ...
        return maxSafeWithdrawWei(aTokenWei, poolLiq, userAccount, reserve);
      }
      case 'borrow':
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'withdraw') {
      // Only use maxUint256 ("withdraw everything") when the user has NO debt.
      // With debt, Aave expands maxUint256 to the full aToken balance which
      // removes all collateral and collapses HF to 0 → revert.
      // When debt exists we pass the exact safe-capped amount instead.
      const hasDebt =
        userAccount !== null &&
        parseFloat(userAccount.totalDebtUSDFull ?? userAccount.totalDebtUSD) > 0;
      const isMaxWithdraw =
        !hasDebt &&
        maxWei !== null &&
        amtWeiParsed !== null &&
        amtWeiParsed >= maxWei;
      await withdraw(asset, normalized, dec, isMaxWithdraw);
    }
  };
